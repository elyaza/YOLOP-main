...
def find_vanishing_point(lines):
    ...
    for i, line1 in enumerate(lines):
        for line2 in lines[i + 1:]:
            ...
            # Convert lines to their slope-intercept form (y = mx + b)
    ...
    # Average the intersections to find the approximate vanishing point
    if len(intersections) > 0:
        vanishing_point = np.mean(intersections, axis=0).astype(int)
        return tuple(vanishing_point)
    else:
        return None

def draw_lanes_with_vanishing_point(image, lines, vanishing_point):
    for line in lines:
        x1, y1, x2, y2 = line[0]
        # Extend the line from the vanishing point
        cv2.line(image, vanishing_point, (x1, y1), (255, 0, 0), 2)
        cv2.line(image, vanishing_point, (x2, y2), (255, 0, 0), 2)
...
def infer_yolop(weight="yolop-640-640.onnx",
                img_path="./inference/images/22.jpg"):
    ...
    save_det_path = f"./inference/output/detect_onnx.jpg"
    save_da_path = f"./inference/output/da_onnx.jpg"
    save_ll_path = f"./inference/output/ll_onnx.jpg"
    save_merge_path = f"./inference/output/output_onnx.jpg"
    save_hough_transform = f"./inference/output/output_hough_transform.jpg"
    ...
    ll_canny = cv2.Canny(ll_seg_mask, 100, 200)  # Apply Canny with lower and upper thresholds
    ...
    cv2.imshow("Lane Line Segmentation", ll_canny_resized)
    cv2.waitKey(0)  # Wait until a key is pressed
    cv2.destroyAllWindows()
    

    # Create a combined image (merge drive area and lane line segments)
    color_area = np.zeros((new_unpad_h, new_unpad_w, 3), dtype=np.uint8)
    color_area[da_seg_mask == 1] = [0, 255, 0]  # Green for drive area
    color_area[ll_seg_mask == 1] = [255, 0, 0]  # Blue for lane lines
    ...
    lines = cv2.HoughLinesP(ll_canny_resized, 1, np.pi / 180, threshold=100, minLineLength=50, maxLineGap=30)
    ...
    # Find the vanishing point
        ...
        # Draw the vanishing point
            cv2.circle(ll_canny_resized, vanishing_point, 5, (0, 0, 255), -1)

        # Draw the lanes passing through the vanishing point
            image_with_lanes = draw_lanes_with_vanishing_point(ll_canny_resized, lines, vanishing_point)

        # Show and save the result
            cv2.imshow('Lanes with Vanishing Point', image_with_lanes)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
        else:
    ...
    cv2.imwrite(save_ll_path,ll_canny_resized )
