...
###### added by me to choose an input image not a random tensor 
# Load and preprocess the input image
image_path = "inference/images/11.jpg"  # Replace with your image path
image = cv2.imread(image_path)  # Read the image
image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB
image = cv2.resize(image, (640, 640))  # Resize to match model input dimensions

# Normalize the image (example for standard ImageNet normalization)
...
# Assume `lane_line_seg` is the tensor output from the model
# Shape: (1, 2, H, W)
    lane_line_seg = outputs[2]  # Example, replace with your actual tensor
    lane_line_seg = np.squeeze(lane_line_seg)  # Remove batch dimension, shape: (2, H, W)

# Extract the first channel (probability map for lane lines)
    lane_line_map = lane_line_seg[0]  # Shape: (H, W)

# Optional: Apply a threshold to create a binary image
    binary_lane_line = (lane_line_map > 0.5).astype(np.uint8) * 255  # Threshold at 0.5, scale to 255

# Display using Matplotlib
    plt.figure(figsize=(10, 5))
    plt.subplot(1, 2, 1)
    plt.title("Lane Line Probability Map")
    plt.imshow(lane_line_map, cmap="gray")

    plt.subplot(1, 2, 2)
    plt.title("Binary Lane Line")
    plt.imshow(binary_lane_line, cmap="gray")
    plt.show()

# Alternatively, save or display using OpenCV
    cv2.imshow("Lane Line Probability Map", (lane_line_map * 255).astype(np.uint8))
    cv2.imshow("Binary Lane Line", binary_lane_line)
    cv2.waitKey(0)
